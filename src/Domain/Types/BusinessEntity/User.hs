module Domain.Types.BusinessEntity.User (
   User(User, avatar, dataCreate, idUser, lastName, nameUser, userIsAdmin,
     userIsAuthor, userLogin, userPassword)
) where
...
import Domain.Types.AuthEntity.Auth (IsAdmin, IsAuthor, Login, Password, UserId)
