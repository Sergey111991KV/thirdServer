...
deleteOldSession us = do
  ...
  case result of
    ...
    _ -> do
      ...
      throwError DataErrorPostgreSQL
  where qry = [sql|delete from session where user_news_id = ?|]
...
insertNewSession uId = do
  ...
  case result of
    ...
    _ -> do
      writeLogE (errorText DataErrorPostgreSQL)
      throwError DataErrorPostgreSQL
  where qry = [sql|INSERT INTO session (key, user_news_id) values (?,?);|]
...
findUserIdBySession sesId = do
  ...
  case result of
    ...
    _ -> do
      writeLogE (errorText DataErrorPostgreSQL)
      throwError DataErrorPostgreSQL
  where qry = [sql|select user_news_id from session where key = ? ;|]
