...
emptyFixture = Fixture { _findUserId            = unimplemented
...
spec = do
  describe "Publish" $ do
    it "should not Publish news because not find user from sessionId" $ do
      ...
      runApp fixture (publishAction (SessionId "Text") 1)
        `shouldReturn` Left DataErrorPostgreSQL
    it "should Publish news" $ do
      let fixture = emptyFixture
            { _findUserIdBySession = \(SessionId "Text") -> return $ UserId 1
            , _publish             = \(UserId 1) 1 -> return ()
            }
      runApp fixture (publish (UserId 1) 1) `shouldReturn` Right ()
    it
        "should not Publish news because wrong userId as not support author in the database"
      $ do
          let fixture = emptyFixture
                { _findUserIdBySession = \(SessionId "Text") ->
                                           return $ UserId 1
                , _publish = \(UserId 1) 1 -> throwError DataErrorPostgreSQL
                }
          runApp fixture (publish (UserId 1) 1)
            `shouldReturn` Left DataErrorPostgreSQL
  ...
  describe "CreateCommon" $ do
    it "should not createCommon with error in postgres" $ do
      ...
      runApp fixture (createCommon (SessionId "Text") mockAuthor)
        `shouldReturn` Left DataErrorPostgreSQL
    it "should not create Author with not admin access" $ do
      ...
      runApp fixture (createCommon (SessionId "Text") mockAuthor)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not create Category with not admin access" $ do
      let fixture = emptyFixture
            { _checkAdminAccess = \_ -> throwError NotAccessNotAdmid
            }
      runApp fixture (createCommon (SessionId "Text") mockCategory)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not create Tag with not admin access" $ do
      let fixture = emptyFixture
            { _checkAdminAccess = \_ -> throwError NotAccessNotAdmid
            }
      runApp fixture (createCommon (SessionId "Text") mockTag)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not create Draft with not author access" $ do
      let fixture = emptyFixture
            { _checkAuthorAccess = \_ -> throwError NotAccessNotAuthor
            }
      runApp fixture (createCommon (SessionId "Text") mockDraft)
        `shouldReturn` Left NotAccessNotAuthor
    ...
    it "should create Draft with author access" $ do
      ...
      runApp fixture (createCommon (SessionId "Text") mockDraftAuthor)
        `shouldReturn` Right ()
    it "should not create Draft with author access because wong author id" $ do
      ...
      runApp fixture (createCommon (SessionId "Text") mockDraft)
        `shouldReturn` Left NotSupposedAuthor
  ...
  describe "editingCommon" $ do
    it "should not editingCommon with error in postgres" $ do
      ...
      runApp fixture (editingCommon (SessionId "Text") mockAuthor)
        `shouldReturn` Left DataErrorPostgreSQL
    ...
    it "should not editing Category with not admin access" $ do
      ...
      runApp fixture (editingCommon (SessionId "Text") mockCategory)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not editing Tag with not admin access" $ do
      let fixture = emptyFixture
            { _checkAdminAccess = \_ -> throwError NotAccessNotAdmid
            }
      runApp fixture (editingCommon (SessionId "Text") mockTag)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not editing Draft with not author access" $ do
      let fixture = emptyFixture
            { _checkAuthorAccess = \_ -> throwError NotAccessNotAuthor
            }
      runApp fixture (editingCommon (SessionId "Text") mockDraft)
        `shouldReturn` Left NotAccessNotAuthor
    ...
    it "should  editing Draft, because it is not this author draft" $ do
      ...
      runApp fixture (editingCommon (SessionId "Text") mockDraftAuthor)
        `shouldReturn` Left NotSupposedAuthor
  ...
  describe "remove" $ do
    it "should not remove with error in postgres" $ do
      ...
      runApp fixture (removeCommon (SessionId "Text") AuthorEntReq 1)
        `shouldReturn` Left DataErrorPostgreSQL
    ...
    it "should not remove Category with not admin access" $ do
      ...
      runApp fixture (removeCommon (SessionId "Text") CategoryEntReq 1)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not remove Tag with not admin access" $ do
      let fixture = emptyFixture
            { _checkAdminAccess = \_ -> throwError NotAccessNotAdmid
            }
      runApp fixture (removeCommon (SessionId "Text") TagEntReq 1)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not remove Draft with not author access" $ do
      let fixture = emptyFixture
            { _checkAuthorAccess = \_ -> throwError NotAccessNotAuthor
            }
      runApp fixture (removeCommon (SessionId "Text") DraftEntReq 1)
        `shouldReturn` Left NotAccessNotAuthor
  ...
  describe "getOne" $ do
    it "should not getOne with error in postgres" $ do
      ...
      runApp fixture (getOneCommon (SessionId "Text") AuthorEntReq 1)
        `shouldReturn` Left DataErrorPostgreSQL
    ...
    it "should not getOne Category with not admin access" $ do
      let fixture =
            emptyFixture { _getOne = \_ _ -> throwError NotAccessNotAdmid }
      runApp fixture (getOneCommon (SessionId "Text") CategoryEntReq 1)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not getOne Tag with not admin access" $ do
      let fixture =
            emptyFixture { _getOne = \_ _ -> throwError NotAccessNotAdmid }
      runApp fixture (getOneCommon (SessionId "Text") TagEntReq 1)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not getOne Draft with not author access" $ do
      let fixture = emptyFixture
            { _checkAuthorAccess = \_ -> throwError NotAccessNotAuthor
            }
      runApp fixture (getOneCommon (SessionId "Text") DraftEntReq 1)
        `shouldReturn` Left NotAccessNotAuthor
    ...
    it "should getOne Category with admin access" $ do
      ...
      runApp fixture (getOneCommon (SessionId "Text") CategoryEntReq 1)
        `shouldReturn` Right "mockCategory"
  ...
  describe "getAll" $ do
    it "should not getAll with error in postgres" $ do
      ...
      runApp fixture (getArrayCommon (SessionId "Text") AuthorEntReq 1)
        `shouldReturn` Left DataErrorPostgreSQL
    ...
    it "should not getAll Category with not admin access" $ do
      let fixture =
            emptyFixture { _getAll = \_ _ -> throwError NotAccessNotAdmid }
      runApp fixture (getArrayCommon (SessionId "Text") CategoryEntReq 1)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not getAll Tag with not admin access" $ do
      let fixture =
            emptyFixture { _getAll = \_ _ -> throwError NotAccessNotAdmid }
      runApp fixture (getArrayCommon (SessionId "Text") TagEntReq 1)
        `shouldReturn` Left NotAccessNotAdmid
    it "should not getAll Draft with not author access" $ do
      let fixture = emptyFixture
            { _checkAuthorAccess = \_ -> throwError NotAccessNotAuthor
            }
      runApp fixture (getArrayCommon (SessionId "Text") DraftEntReq 1)
        `shouldReturn` Left NotAccessNotAuthor
