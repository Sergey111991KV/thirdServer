...
spec = do
  describe "Create session" $ do
    it "should not create session because not find user from login and password"
      $ do
          let
            fixture = emptyFixture
              { _findUserId = \_ _ -> throwError DataErrorPostgreSQL
              }
          runApp fixture (sessionByAuth (Login "Text") (Password "Text"))
            `shouldReturn` Left DataErrorPostgreSQL
    it "should create session " $ do
      let fixture = emptyFixture
            { _findUserId = \(Login "Text") (Password "Text") ->
                              return $ UserId 1
            , _newSession = \(UserId 1) -> return $ SessionId "Some Text"
            }
      runApp fixture (sessionByAuth (Login "Text") (Password "Text"))
        `shouldReturn` Right (SessionId "Some Text")
  describe "Delete session" $ do
    it "should not delete old session because not find user from sessionId" $ do
      ...
      runApp fixture (exitSession (SessionId "Test"))
        `shouldReturn` Left DataErrorPostgreSQL
    it "should  delete old session" $ do
      ...
      runApp fixture (exitSession (SessionId "Test")) `shouldReturn` Right ()
